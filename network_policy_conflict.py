...
def check_policy_for_similars(policy, pyt_src, pyt_dst):
    ...
    # 添加policy到patricia
    # self.add_policy_to_patricia(policy, pyt_src, pyt_dst)
    ...
    final_policys = list(set(src_conflict_policys)& set(dst_conflict_policys))
    return final_policys,src_same_conflict_policys,src_child_conflict_policys,src_parent_conflict_policys,dst_same_conflict_policys,dst_child_conflict_policys,dst_parent_conflict_policys
...
def subset_for_ip(pyt_src,pyt_dst,con_policy,policy,src_same_conflict_rules,src_child_conflict_rules,src_paren_conflict_rules,dst_same_conflict_rules,dst_child_conflict_rules,dst_paren_conflict_rules):
    ...
    if ((compare in src_child_conflict_rules) and (compare in dst_paren_conflict_rules)):
        conflict_type = 'intersect'
        src_intersection_part = con_policy['src_ip']
        dst_intersection_part = policy['dst_ip']
    if ((compare in src_paren_conflict_rules) and (compare in dst_child_conflict_rules)):
        conflict_type = 'intersect'
        src_intersection_part = policy['src_ip']
        dst_intersection_part = con_policy['dst_ip']
    ...
    if ((check_tcp_udp(policy)!=check_tcp_udp(con_policy)) or (final_type == 'different')):
        ...
        if(policy['action'] == con_policy['action']):
            ...
            if (policy['priority'] == con_policy['priority']):
                ...
                print(policy['assno'],"has Sheilding conflict with ",con_policy['aasno'])
        ...
        if (policy['action'] == con_policy['action']):
            # 冗余冲突
            print(policy['aasnno'], "has Intersection_same_action conflict with ", con_policy['aasno'])
        else:
            if (policy['priority'] == con_policy['priority']):
                print(policy['aasno'], "has Intersection_different_action with same priority conflict with ", con_policy['aasno'])
            else:
                print(policy['assno'], "has Intersection_different_action conflict with ", con_policy['aasno'])
    print("***********************")
    return rap


def detection(policys, pyt_src, pyt_dst):
    print("Detection Policy Conflict：")
    for policy in policys:
        if check_layer2_layer4(policy) == True:
            print("\nReconcile %s" % policy['aasno'])
            pass
        else:
            print("\nNo Recon %s" % policy['aasno'])
            conflict_policy_numbers, src_same_conflict_rules, src_child_conflict_rules, src_paren_conflict_rules, dst_same_conflict_rules,\
                dst_child_conflict_rules, dst_paren_conflict_rules = check_policy_for_similars(policy, pyt_src, pyt_dst, rap)
            # print("Conflicted_numbers:",conflict_policy_numbers)
            if len(conflict_policy_numbers) == 0:
                ...
                """
                rap = 100
                for i in conflict_policy_numbers:
                    it = str(i)
                    my_item = 100
                    for item in fd:
                        if item["aasno"] == it:
                    ...
                    if my_item != 100:
                        con_policy = my_item
                        rap1 = detection_algorithm(con_policy,policy,pyt_src,pyt_dst,src_same_conflict_rules,src_child_conflict_rules, src_paren_conflict_rules,dst_same_conflict_rules,dst_child_conflict_rules,dst_paren_conflict_rules,rap)
                    else:
                        pass
